...
StyleSwitcher.prototype.build = function () {
    ...
    $('body').css({
        'background-image': 'url(' + imgPath + 'assets/images/pattern/' + $this.storageManager.getBgImage() + '.png)',
    ...
    $.each(patternImages, function (i) {
        var listElement = $('<li/>').append($('<a/>').css({
                'background': 'url(' + imgPath + 'assets/images/pattern/' + patternImages[i].image + '.png) repeat'
            }).attr({
                'href': '#',
    ...
    patternList.find('a').on(Metis.buttonPressedEvent, function (e) {
        e.preventDefault();
        $('body').css({
            'background-image': 'url(' + imgPath + 'assets/images/pattern/' + $(this).data('patternImage') + '.png)',
            'background-repeat': ' repeat'
        });
